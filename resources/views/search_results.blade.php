...
@section('konten2')
<div class="container">
    <h3 style="margin-top: 20px; margin-bottom: 50px; text-align: center;">Pencarian Data Pengaduan</h3>
    <style>
         .table-container {
        ...
        .table-container table td:first-child {
            ...
            <form action="{{ route('search') }}" method="GET">
                <div class="form-group">
                    ...
                    <div class="form-text mt-2">Kata kunci pencarian: Unit Ruangan, Nama, Masalah, Isi Masalah, Solusi, Status, dan Kategori.</div>
                </div>
                        ...
                        <tbody>
                            @foreach($searchResults as $index => $result)
                                <tr>
                                    ...
                                    <td>{{ $result->nama }}</td>
                                    <td>{{ $result->media }}</td>
                                    <td>{{ implode(', ', explode(',', $result->masalah)) }}</td>
                                    ...
                                    <td>{{ implode(', ', explode(',', $result->solusi)) }}</td>
                                    <td>{{ $result->status }}</td>
                                    ...
                                    <div>
                                        <form action="{{ route('delete', $result->id) }}" method="POST" style="display: inline;">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    <div>
                                        <a href="{{ route('edit', $result->id) }}" class="btn btn-outline-warning">EDIT</a>
                                </div>
                            </td>
                                </tr>
                            @endforeach
